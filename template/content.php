<?php 
	...
	while ( $loop->have_posts() ) : $loop->the_post();
		...
		<?php  if($colorbox_sec_title == 'yes' ) { ?>
				...
				<div id="colorbox_main_container_<?php echo esc_attr($post_id);  ?>" style="colorbox_main_container">
					<?php
					...
					?>
					<div class=" wpsm_col-md-<?php echo esc_attr($box_layout); ?> wpsm_col-sm-6 colorbox_singel_box">
						<div class="wpsm_panel wpsm_panel-default wpsm_panel_default_<?php echo esc_attr($post_id); ?> ">
						  ...
						  <div class="wpsm_panel-heading">
							...
							{ 
								...
								{
							?>
									<span style="margin-right:6px;" class="fa <?php echo esc_attr($colorbox_title_icon); ?>"></span>
							<?php
								}
							}
							if($show_colorbox_title_icon == '1' || $show_colorbox_title_icon==2 )
							{  echo esc_html($colorbox_title); } ?></h3>
						  </div> <?php } ?>
							<?php 
				...
				<script>
					jQuery(window).load(function(){
						jQuery('#colorbox_main_container_<?php echo esc_attr($post_id);  ?>').masonry({
				...
				<script>
					var col = new jColumn();
					col.jcolumn('wpsm_panel_default_<?php echo esc_attr($post_id); ?>');
				</script>
				<?php } ?>
			<?php
		}
