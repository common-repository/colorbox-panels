...
<style>
				...
				if($TotalCount!=-1)
				{
					foreach($colorbox_data as $colorbox_single_data)
					{
						...
						<li class="wpsm_ac-panel single_color_box" >
							...
							<a type="button" class="btn btn-primary btn-block html_editor_button" data-remodal-target="modal" href="#" id="<?php echo esc_attr($i); ?>"  onclick="open_editor(<?php echo esc_attr($i); ?>)"><?php esc_html_e('Use WYSIWYG Editor',wpshopmart_colorbox_text_domain); ?></a>
							
							
							<span class="ac_label"><?php _e('Box Icon',wpshopmart_colorbox_text_domain); ?></span>
							<div class="form-group input-group">
								<input data-placement="bottomRight" id="colorbox_title_icon[]" name="colorbox_title_icon[]" class="form-control icp icp-auto" value="<?php echo esc_attr($colorbox_title_icon); ?>" type="text" readonly="readonly" />
							...
							<select name="enable_single_icon[]" style="width:100%" >
								<option value="yes" <?php if($enable_single_icon == 'yes') echo "selected=selected"; ?>><?php esc_html_e('Yes',wpshopmart_colorbox_text_domain); ?></option>
								<option value="no" <?php if($enable_single_icon == 'no') echo "selected=selected"; ?>><?php esc_html_e('No',wpshopmart_colorbox_text_domain); ?></option>
			...
			else 
			{
				  for($i=1; $i<=2; $i++)
				  {
					 ...
					 <li class="wpsm_ac-panel single_color_box" >
						...
						<select name="enable_single_icon[]" style="width:100%" >
								<option value="yes" selected=selected><?php esc_html_e('Yes',wpshopmart_colorbox_text_domain); ?></option>
								<option value="no" ><?php esc_html_e('No',wpshopmart_colorbox_text_domain); ?></option>
						</select>
						<a class="remove_button" href="#delete" id="remove_bt" ><i class="fa fa-trash-o"></i></a>
	...
	<div class="remodal" data-remodal-options=" closeOnOutsideClick: false" data-remodal-id="modal" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc">
	  ...
	  <div>
		<h2 id="modal1Title"><?php esc_html_e('Colorbox Editor',wpshopmart_colorbox_text_domain); ?></h2>
		<p id="modal1Desc">
		  <?php
			$content = '';
			$editor_id = 'get_text';
	  ...
	  <br>
	  <button data-remodal-action="cancel" class="remodal-cancel"><?php esc_html_e('Cancel',wpshopmart_colorbox_text_domain); ?></button>
	  <button data-remodal-action="confirm" class="remodal-confirm" onclick="insert_html()"><?php esc_html_e('OK',wpshopmart_colorbox_text_domain); ?></button>
	</div>
...
<h1><?php esc_html_e('Get Support',wpshopmart_colorbox_text_domain); ?></h1>
<h3><?php esc_html_e('If You have any issue then please ask us any time',wpshopmart_colorbox_text_domain); ?></h3>
<a href="https://wordpress.org/support/plugin/colorbox-panels" target="_blank" class="button button-primary button-hero "><?php esc_html_e('Submit Your Support Ticket',wpshopmart_colorbox_text_domain); ?></a>
<br /> <br />
